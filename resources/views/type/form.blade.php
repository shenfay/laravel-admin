@empty($type->id)
<form autocomplete="off" class="layui-form layui-box modal-form-box" action="{{ route('admin.types.store', ['model'=>$class]) }}" data-auto="true" method="POST">
@else
    <form autocomplete="off" class="layui-form layui-box modal-form-box" action="{{ route('admin.types.update', $type->id) }}" data-auto="true" method="PUT">
        ...
        @empty($type->id)<input type="hidden" name="model" value="{{ $model }}" />@endif
